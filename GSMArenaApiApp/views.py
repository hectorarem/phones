...
class PhoneDetailViewSet(viewsets.ModelViewSet):
    ...
    def data_cleaning(self):
        for m in Phone.objects.all():
            print(m.name)
            m.name = m.name.split("</title>")[0].replace(
                ' - Full tablet specifications', '').replace(" - Full phone specifications", '')
            m.save()
    ...
    def get_queryset(self):
        ...
        if queryset.count() > 0:
            ...
            try:
                ...
                try:
                    name = phoneScrap.text.split("<title>")[1].split("</title>")[0].replace(
                        ' - Full tablet specifications', '').replace(" - Full phone specifications", '')
                except:
                    name = ""
                    pass
